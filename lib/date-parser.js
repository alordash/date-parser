...
function setDateProperty(date, parsedTime, timeType) {
	if (typeof (date) == 'undefined') {
		if (parsedTime.dateType == DateTypes.period) {
			date = new Date(0);
		} else {
			date = new Date();
		}
	...
	switch (timeType) {
		...
		case TimeTypes.months:
			date.setMonth(parsedTime.number - 1);
			break;
...
function parseDate(string, errorLimit, minimumPrevalence) {
	...
	for (timeSet of timeSets) {
		...
		if(typeof(result.period_time) != 'undefined') {
			result.period_time = result.period_time.getTime() / 1000;
		}
