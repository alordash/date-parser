 ...
 * @param {Boolean} checkChar 
 * @returns {Boolean}
 */
function IsSuitableContext(thisContext, targetContext, contextsData, expressions, checkChar) {
   const step = (targetContext > thisContext ? 1 : 0);
   const start = Math.min(thisContext, targetContext);
   const end = Math.max(thisContext, targetContext);
   const startContext = contextsData.contexts[start];
   if (expressions[startContext.end].regex_char == 'I' || !checkChar) {
      for (let i = start + step; i < end + step; i++) {
 ...
 * @param {Boolean} atEnd
 */
function addWordsFromContext(parsedDateIndex, index, contextsData, expressions, atEnd) {
   let lastChar;
   if(atEnd) {
      lastChar = expressions[contextsData.contexts[index - 1].end].text;
   } else {
      lastChar = expressions[contextsData.contexts[index].start].text;
   }
   ...
   if (lastChar == ',') {
      ...
      for (let i = context.start; i <= context.end; i++) {
         string += expressions[i].text + ' ';
      }
      string = string.trim();
      if(atEnd) {
         this[parsedDateIndex].string += ', ' + string;
      } else {
         this[parsedDateIndex].string = string + ' ' + this[parsedDateIndex].string;
      }
   } else if (lastChar == '.') {
      let string = '';
      let context = contextsData.contexts[index];
      ...
      string = string.trim();
      if(atEnd) {
         this[parsedDateIndex].string += '. ' + string[0].toUpperCase() + string.substring(1);
      } else {
         this[parsedDateIndex].string = string  + ' ' + this[parsedDateIndex].string[0].toUpperCase() + this[parsedDateIndex].string.substring(1);
      }
   }
}
 ...
 * @param {Array.<Expression>} expressions 
 */
function fillNotUsedContexts(notUsedContexts, contextsData, expressions) {
   for (const index of notUsedContexts) {
      let proceed = true;
      if (index > 0) {
         let parsedDateIndex = findParsedDateByContext(index - 1, this);
         if (parsedDateIndex != -1) {
            addWordsFromContext.call(this, parsedDateIndex, index, contextsData, expressions, true);
            proceed = false;
         }
      }
      if (index < contextsData.contexts.length - 1 && proceed) {
         let parsedDateIndex = findParsedDateByContext(index + 1, this);
         addWordsFromContext.call(this, parsedDateIndex, index, contextsData, expressions, false);
      }
   }
}
...
function parseDate(string, errorLimit, minimumPrevalence) {
   ...
   for (timeSet of timeSets) {
      ...
      for (let j = 0; j < expressions.length; j++) {
         ...
         if (indexes.indexOf(j) == -1
            && ((start <= j && j <= end) || IsSuitableContext(timeSet.context, expression.context, contextsData, expressions, true))) {
            expression.index = j;
   ...
   fillNotUsedContexts.call(parsedDates, notUsedContexts, contextsData, expressions)
   return parsedDates;
}
