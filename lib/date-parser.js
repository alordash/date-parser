...
const { isDateType, isTimeType, setDateProperty, ValidModes, ParsedTime, ContextsData, extractTime } = require('./date-cases');
...
function parseDate(string, errorLimit, minimumPrevalence) {
   ...
   for (let parsedDate of parsedDates) {
      ...
      for (const timeType in parsedDate.target_date) {
         const val = parsedDate.target_date[timeType];
