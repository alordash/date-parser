...
const { isDateType, setDateProperty, ValidModes, ParsedTime, ContextsData, extractTime } = require('./date-cases');
...
class ParsedDate {
    ...
    * @param {Array.<Number>} contexts 
	 */
   constructor(target_date, period_time, max_date, string, confidence, contexts, isOffset) {
...
function parseDate(string, errorLimit, minimumPrevalence) {
   ...
   for (let parsedDate of parsedDates) {
      ...
      for (const timeType in parsedDate.max_date) {
         if (typeof (parsedDate.max_date[timeType]) != 'undefined') {
            parsedDate.max_date[timeType] = parsedDate.max_date[timeType].number;
         }
      }
      for (const timeType in parsedDate.period_time) {
         if (typeof (parsedDate.period_time[timeType]) != 'undefined') {
            parsedDate.period_time[timeType] = parsedDate.period_time[timeType].number;
         }
      }
      for (const timeType in parsedDate.target_date) {
         if (typeof (parsedDate.target_date[timeType]) != 'undefined') {
            parsedDate.target_date[timeType] = parsedDate.target_date[timeType].number;
         }
      }
   }
   return parsedDates;
