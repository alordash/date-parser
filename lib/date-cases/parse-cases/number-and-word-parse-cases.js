...
const parseCases = [
   ...
   new ParseCase(prevalence, function (parsedTimes, contextsData, prevalence) {
      ...
      for (const match of matches) {
         ...
         let context = processContexts(contextsData, indexes);
         parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.hours, num, indexes, context, prevalence, { validMode, isFixed }));
      }
      return parsedTimes;
   }),
   new ParseCase(prevalence, function (parsedTimes, contextsData, prevalence) {
   ...
   new ParseCase(prevalence, function (parsedTimes, contextsData, prevalence) {
      ...
      for (const match of matches) {
         let num = parseFloat(this.expressions[match.index].text) * 7;
         let max = this.expressions[match.index + match[0].length - 1].maximum;
         let validMode = ValidModes.notCertified;
         if (num > max && max != 0) {
            validMode = ValidModes.notValid;
   ...
   new ParseCase(prevalence, function (parsedTimes, contextsData, prevalence) {
      let matches = [...this.regchars.matchAll(/Mn/g)];
      for (const match of matches) {
         let num = parseFloat(this.expressions[match.index + 1].text);
         let month = this.expressions[match.index];
         let max = month.maximum;
         let validMode = ValidModes.notCertified;
         if (num > max && max != 0) {
            validMode = ValidModes.notValid;
         }
         ...
         let context = processContexts(contextsData, indexes);
         parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.dates, num, indexes, context, prevalence, { validMode }));
         parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.months, month.value, indexes, context, prevalence, { validMode }));
      }
      return parsedTimes;
   }),
   new ParseCase(prevalence, function (parsedTimes, contextsData, prevalence) {
   ...
   new ParseCase(prevalence, function (parsedTimes, contextsData, prevalence) {
      ...
      for (const match of matches) {
         ...
         let num = parseFloat(text.substring(0, text.length - 1));
   ...
   new ParseCase(prevalence, function (parsedTimes, contextsData, prevalence) {
      ...
      for (const match of matches) {
         ...
         let isFixed = false
         if (value != 0) {
            num = processHour(num, this.expressions[match.index].value);
            isFixed = true;
         }
         let max = this.expressions[match.index].maximum;
         let validMode = ValidModes.notCertified;
