...
class ParsedTime {
    ...
    * @param {Boolean} isOffset 
	 */
   constructor(dateType, timeType, number, indexes, context, prevalence, validMode, isOffset) {
         ...
         if (parsedTimesIndexes.length > 0) {
            ...
            while (i--) {
               ...
               if (parsedTime.timeType == TimeTypes.nMonth) {
                  parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.months, now.getMonth() + 1 + parsedTime.number, parsedTime.indexes, context, prevalence, undefined, true));
            ...
            for (const timeType of notFoundTimeTypes) {
               parsedTimes.push(new ParsedTime(DateTypes.target, timeType, getDateProperty(now, timeType), indexes, context, prevalence, undefined, true));
            }
         }
      }
      return parsedTimes;
