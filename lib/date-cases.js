 ...
 * @returns {Number}
 */
function getDateProperty(date, timeType) {
	switch (timeType) {
 ...
 * @returns {Number} 
 */
function getDateMultiplication(timeType) {
	switch (timeType) {
		case TimeTypes.seconds:
			return 1;
		case TimeTypes.minutes:
...
const parseCases = [
	...
	new ParseCase(60, function (parsedTimes, contextsData, prevalence) {
		...
		for (const match of matches) {
			...
			const now = new Date();
			if (value == 0) {
				parsedTimes.push(new ParsedTime(DateTypes.period, TimeTypes.seconds, 24 * 60 * 60, indexes, context, prevalence));
				parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.dates, now.getDate() + 1, indexes, context, prevalence));
			} else {
				let dif = value - (now.getDay() + 1);
				if (dif < 0) {
					dif += 7;
				}
				parsedTimes.push(new ParsedTime(DateTypes.period, TimeTypes.seconds, 7 * 24 * 60 * 60, indexes, context, prevalence));
				parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.dates, now.getDate() + dif, indexes, context, prevalence));
			}
		}
		return parsedTimes;
 ...
 * @returns {Array.<Number>}
 */
function findAllMatchingParsedTimes(allowedTypes, step, start, expressions, parsedTimes) {
	let i = start + step;
	let proceed = true;
	let result = [];
	while (proceed && (0 <= i && i < expressions.length)) {
		...
		if (indexes.length > 0) {
			...
			for (const index of indexes) {
				...
				if (allowedTypes.includes(parsedTime.timeType)) {
...
const finalParseCases = [
	...
	new ParseCase(90, function (parsedTimes, contextsData, prevalence) {
		...
		for (const match of matches) {
			...
			if (parsedTimesIndexes.length > 0) {
				...
				parsedTimes[parsedTimesIndexes[i]].indexes.push(...markIndexes.call(this, match.index, 1, true));
			}
		}
		return parsedTimes;
	}),
	...
	new ParseCase(90, function (parsedTimes, contextsData, prevalence) {
		...
		for (const match of matches) {
			...
			if (parsedTimes.length > 0) {
				let i = parsedTimesIndexes.length;
				const now = new Date();
				const indexes = markIndexes.call(this, match.index, 1, true);
				...
				while (i--) {
					...
					if (parsedTime.timeType == TimeTypes.nMonth) {
						parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.months, now.getMonth() + 1 + parsedTime.number, parsedTime.indexes, context, prevalence));
						parsedTimes.splice(parsedTimesIndexes[i], 1);
						notFoundTimeTypes.splice(notFoundTimeTypes.indexOf(TimeTypes.months), 1);
					} else {
						parsedTime.number = getDateProperty(now, parsedTime.timeType) + parsedTime.number;
					}
					parsedTime.prevalence += prevalence;
					parsedTime.indexes.push(...indexes);
				}
				for (const timeType of notFoundTimeTypes) {
					parsedTimes.push(new ParsedTime(DateTypes.target, timeType, getDateProperty(now, timeType), indexes, context, prevalence));
				}
			}
		}
		return parsedTimes;
	...
	new ParseCase(90, function (parsedTimes, contextsData, prevalence) {
		let matches = [...this.regchars.matchAll(/V\.+I{0,1}Z\.+/g)];
		for (const match of matches) {
			const allowedTypes = [TimeTypes.years, TimeTypes.dates, TimeTypes.months, TimeTypes.hours, TimeTypes.minutes, TimeTypes.seconds];
			let fromParsedTimesIndexes = findAllMatchingParsedTimes(allowedTypes, 1, match.index, this.expressions, parsedTimes).sort();
			if (fromParsedTimesIndexes.length > 0) {
				let notFoundTTFrom = [TimeTypes.dates, TimeTypes.hours, TimeTypes.minutes, TimeTypes.months, TimeTypes.seconds, TimeTypes.years];
				let proceed = true;
				for (const i of fromParsedTimesIndexes) {
					let parsedTime = parsedTimes[i];
					let index = notFoundTTFrom.indexOf(parsedTime.timeType);
					if (index != -1) {
						notFoundTTFrom.splice(index, 1);
					} else {
						proceed = false;
						break;
					}
				...
				if (proceed) {
					let zIndex = match[0].indexOf('Z');
					let toParsedTimesIndexes = findAllMatchingParsedTimes(allowedTypes, 1, match.index + zIndex, this.expressions, parsedTimes).sort();
					if (toParsedTimesIndexes.length > 0) {
						let notFoundTTTo = [TimeTypes.dates, TimeTypes.hours, TimeTypes.minutes, TimeTypes.months, TimeTypes.seconds, TimeTypes.years];
						for (const i of toParsedTimesIndexes) {
							let parsedTime = parsedTimes[i];
							let index = notFoundTTTo.indexOf(parsedTime.timeType);
							if (index != -1) {
								notFoundTTTo.splice(index, 1);
							} else {
								proceed = false;
								break;
							}
						...
						if (proceed) {
							...
							for (const i of fromParsedTimesIndexes) {
								let parsedTime = parsedTimes[i];
								parsedTime.dateType = DateTypes.target;
								parsedTime.indexes.push(...indexes);
							}
							for (const timeType of notFoundTTFrom) {
								parsedTimes.push(new ParsedTime(DateTypes.target, timeType, getDateProperty(now, timeType), indexes, context, prevalence));
							}
							indexes = markIndexes.call(this, match.index + zIndex, 1, true);
							context = processContexts(contextsData, indexes);
							for (const i of toParsedTimesIndexes) {
								let parsedTime = parsedTimes[i];
								parsedTime.dateType = DateTypes.max;
								parsedTime.indexes.push(...indexes);
							}
							for (const timeType of notFoundTTTo) {
								parsedTimes.push(new ParsedTime(DateTypes.max, timeType, getDateProperty(now, timeType), indexes, context, prevalence));
							}
						}
					}
				}
	...
	new ParseCase(90, function (parsedTimes, contextsData, prevalence) {
		...
		for (const match of matches) {
			const allowedTypes = [TimeTypes.dates, TimeTypes.hours, TimeTypes.minutes, TimeTypes.seconds];
			...
			if (parsedTimes.length > 0) {
				...
				while (i--) {
					...
					difSum += getDateMultiplication(parsedTime.timeType) * parsedTime.number;
					parsedTimes.splice(index, 1);
				}
				const context = processContexts(contextsData, indexes);
				if (difSum != 0) {
					parsedTimes.push(new ParsedTime(DateTypes.period, TimeTypes.seconds, difSum, indexes, context, prevalence));
				}
			}
		}
		return parsedTimes;
