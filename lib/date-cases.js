	...
	new ParseCase(90, function (parsedTimes, contextsData, prevalence) {
		...
		for (const match of matches) {
			...
			if (
				...
				let indexes = markIndexes.call(this, match.index, 1, true);
				let context = processContexts(contextsData, indexes);
				parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.dates, date, 0, indexes, context, 90));
				parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.months, 0, month, indexes, context, 90));
				parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.years, year, 0, indexes, context, 90));
			}
		}
		return parsedTimes;
	})
