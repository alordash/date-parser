 ...
 * @returns {Number}
 */
function findParsedTimeByIndex(parsedTimes, index) {
...
function findAllMatchingParsedTimes(allowedTypes, step, start, expressions, parsedTimes) {
	...
	while (proceed && (0 <= i && i < expressions.length)) {
		let index = findParsedTimeByIndex(parsedTimes, i);
		if (index != -1) {
			const parsedTime = parsedTimes[index];
			if (allowedTypes.includes(parsedTime.timeType)) {
				result.push(index);
				if (step > 0) {
					i = parsedTime.indexes[parsedTime.indexes.length - 1] + step;
				} else {
					i = parsedTime.indexes[0] + step;
				}
			} else {
				proceed = false;
			}
