...
const parseCases = [
   ...
   new ParseCase(90, function (parsedTimes, contextsData, prevalence) {
      ...
      for (const match of matches) {
         ...
         if (1 <= hours <= 25 && 0 < minutes && minutes < 60) {
            ...
            let context = processContexts(contextsData, indexes);
            parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.hours, hours, indexes, context, prevalence, { isFixed }));
            parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.minutes, minutes, indexes, context, prevalence));
         }
      }
      return parsedTimes;
   ...
   new ParseCase(90, function (parsedTimes, contextsData, prevalence) {
      ...
      for (const match of matches) {
         ...
         let context = processContexts(contextsData, indexes);
         const now = new Date();
         parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.hours, now.getUTCHours(), indexes, context, prevalence, { isOffset: true }));
