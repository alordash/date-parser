...
let parseCases = [
    new ParseCase(10, function findSeconds(parsedTimes, prevalence) {
        ...
        for (const match of matches) {
            ...
            parsedTimes.push(new ParsedTime(DateTypes.target, "seconds", +this.expressions[match.index].text, this.expressions[match.index + 1].value, indexes, prevalence));
        }
        return parsedTimes;
    })
    ,
    new ParseCase(10, function findMinutes(parsedTimes, prevalence) {
        ...
        for (const match of matches) {
            let indexes = markIndexes.call(this, match.index, 2, true);
            parsedTimes.push(new ParsedTime(DateTypes.target, "minutes", +this.expressions[match.index].text, this.expressions[match.index + 1].value, indexes, prevalence));
        }
        return parsedTimes;
    }),
    new ParseCase(10, function findDates(parsedTimes, prevalence) {
        ...
        for (const match of matches) {
            let indexes = markIndexes.call(this, match.index, 2, true);
            parsedTimes.push(new ParsedTime(DateTypes.target, "dates", +this.expressions[match.index].text, this.expressions[match.index + 1].value, indexes, prevalence));
        }
        return parsedTimes;
    }),
    new ParseCase(10, function findMonths(parsedTimes, prevalence) {
        ...
        for (const match of matches) {
            let indexes = markIndexes.call(this, match.index, 2, true);
            parsedTimes.push(new ParsedTime(DateTypes.target, "months", +this.expressions[match.index].text, this.expressions[match.index + 1].value, indexes, prevalence));
        }
        return parsedTimes;
    }),
    new ParseCase(10, function findYears(parsedTimes, prevalence) {
        ...
        for (const match of matches) {
            let indexes = markIndexes.call(this, match.index, 2, true);
            parsedTimes.push(new ParsedTime(DateTypes.target, "years", +this.expressions[match.index].text, this.expressions[match.index + 1].value, indexes, prevalence));
        }
        return parsedTimes;
    }),
