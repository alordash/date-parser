...
const parseCases = [
	...
	new ParseCase(60, function (parsedTimes, contextsData, prevalence) {
		let matches = [...this.regchars.matchAll(/EO/g)];
		for (const match of matches) {
			let value = this.expressions[match.index + 1].value;
			if (value == 3) {
				let indexes = markIndexes.call(this, match.index, 2, true);
				...
				parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.dates, now.getDate() + 1, indexes, context));
			}
		}
		return parsedTimes;
	}),
	//Searchs for "every day of week" cases
	new ParseCase(60, function (parsedTimes, contextsData, prevalence) {
		...
		for (const match of matches) {
			...
			parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.dates, now.getDate() + dif, indexes, context));
		}
		return parsedTimes;
	})
];
