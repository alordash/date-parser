      ...
      for (const match of matches) {
         ...
         if (parsedTimesIndexes.length > 0) {
            ...
            for (const timeType of notFoundTimeTypes) {
               parsedTimes.push(new ParsedTime(DateTypes.target, timeType, getDateProperty(now, timeType, true), indexes, context, prevalence, undefined, true));
            }
         }
      }
      return parsedTimes;
      ...
      for (const match of matches) {
         ...
         if (parsedTimes.length > 0) {
            ...
            while (i--) {
               ...
               parsedTime.validMode = ValidModes.certified;
               parsedTime.prevalence += prevalence;
               if (parsedTime.timeType == TimeTypes.dates) {
                  parsedTime.number++;
               }
            }
         }
