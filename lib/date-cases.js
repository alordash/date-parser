...
const parseCases = [
   ...
   new ParseCase(90, function (parsedTimes, contextsData, prevalence) {
      ...
      for (const match of matches) {
         ...
         if (1 <= num <= 25 && 0 < minutes && minutes < 60) {
            ...
            let context = processContexts(contextsData, indexes);
            parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.hours, num, indexes, context, prevalence, { isFixed }));
            parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.minutes, minutes, indexes, context, prevalence));
         }
      }
      return parsedTimes;
   ...
   new ParseCase(75, function (parsedTimes, contextsData, prevalence) {
      ...
      for (const match of matches) {
         ...
         if (checkHours(num)) {
            ...
            let indexes = markIndexes.call(this, match.index, match[0].length, true, true);
            let context = processContexts(contextsData, indexes);
            parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.hours, num, indexes, context, prevalence, { isFixed: true }));
         }
      }
      return parsedTimes;
   }),
   ...
   new ParseCase(90, function (parsedTimes, contextsData, prevalence) {
      ...
      for (const match of matches) {
         ...
         let indexes = markIndexes.call(this, match.index, 2, false, true);
         let context = processContexts(contextsData, indexes);
         const now = new Date();
         parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.dates, now.getDate() + value, indexes, context, prevalence, { isOffset: true }));
         parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.months, now.getMonth() + 1, indexes, context, prevalence, { isOffset: true }));
         
         const notFoundTimeTypes = [TimeTypes.hours, TimeTypes.minutes, TimeTypes.seconds, TimeTypes.years];
   ...
   new ParseCase(90, function (parsedTimes, contextsData, prevalence) {
      ...
      for (const match of matches) {
         ...
         let context = processContexts(contextsData, indexes);
         const now = new Date();
         parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.hours, now.getHours(), indexes, context, prevalence, { isOffset: true }));
         parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.minutes, now.getMinutes() + 30, indexes, context, prevalence, { isOffset: true }));
   ...
   new ParseCase(60, function (parsedTimes, contextsData, prevalence) {
      ...
      for (const match of matches) {
         ...
         if (value == 0) {
            ...
            parsedTimes.push(new ParsedTime(DateTypes.period, TimeTypes.dates, 7, indexes, context, prevalence));
            parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.dates, now.getDate() + dif, indexes, context, prevalence));
         }
      }
      return parsedTimes;
   ...
   while (proceed && (0 <= i && i < expressions.length)) {
      ...
      if (indexes.length > 0) {
         ...
         for (const index of indexes) {
            ...
            if (allowedTypes.includes(parsedTime.timeType) && parsedTime.validMode != ValidModes.certified) {
      ...
      for (const match of matches) {
         ...
         if (fromParsedTimesIndexes.length > 0) {
            ...
            if (proceed) {
               ...
               let toParsedTimesIndexes = findAllMatchingParsedTimes(allowedTypes, 1, zIndex, this.expressions, parsedTimes).sort();
