...
class ParsedTime {
	 ...
	 */
	constructor(dateType, timeType, number, indexes, context, prevalence, certified) {
	...
	new ParseCase(60, function (parsedTimes, contextsData, prevalence) {
		...
		for (const match of matches) {
			...
			if (value == 0) {
				...
				parsedTimes.push(new ParsedTime(DateTypes.period, TimeTypes.seconds, dif, indexes, context, prevalence));
				parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.dates, now.getDate() + dif, indexes, context, prevalence));
			}
		}
		return parsedTimes;
	...
	while (proceed && (0 <= i && i < expressions.length)) {
		...
		if (indexes.length > 0) {
			...
			for (const index of indexes) {
				...
				if (allowedTypes.includes(parsedTime.timeType) && !parsedTime.certified) {
	...
	new ParseCase(90, function (parsedTimes, contextsData, prevalence) {
		...
		for (const match of matches) {
			...
			if (fromParsedTimesIndexes.length > 0) {
				...
				if (proceed) {
					let zIndex = match.index + match[0].indexOf('Z');
