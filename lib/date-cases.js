...
class ParsedTime {
   ...
   constructor(dateType, timeType, number, indexes, context, prevalence, options) {
      ...
      if (typeof (options) != 'undefined') {
         ...
         this.owner = typeof (options.owner) == 'undefined' ? undefined : options.owner;
         this.dependents = typeof (options.dependents) == 'undefined' ? [] : options.dependents;
      } else {
...
const numberAndWordParseCases = [
   new ParseCase(50, function (parsedTimes, contextsData, prevalence) {
      ...
      for (const match of matches) {
         ...
         this.regchars = `${regchars.substring(0, match.index)}nh${regchars.substring(match.index + 1)}`;
         let expression = this.expressions[match.index];
         expression.regex_char = 'n';
         expression.is_regex = true;
         expression.text = '0.5';
         this.expressions.splice(match.index + 1, 0, new Expression('~', 'h', 0, 0, 0, false, new ConvertedWord('~', [])));
      }
      return parsedTimes;
   }),
   new ParseCase(50, function (parsedTimes, contextsData, prevalence) {
      ...
      for (const match of matches) {
         let regchars = this.regchars;
         this.regchars = `${regchars.substring(0, match.index)}n${regchars.substring(match.index + 1)}`;
         let expression = this.expressions[match.index];
         expression.regex_char = 'n';
         expression.is_regex = true;
         expression.text = '0.5';
...
const finalParseCases = [
   ...
   new ParseCase(80, function (parsedTimes, contextsData, prevalence) {
      ...
      for (const match of matches) {
         ...
         let parsedTimesIndexes = findAllMatchingParsedTimes(allowedTypes, -1, match.index, this.expressions, parsedTimes, ValidModes.none).sort();
         if (parsedTimesIndexes.length == 0) {
            parsedTimesIndexes = findAllMatchingParsedTimes(allowedTypes, 1, match.index + length - 1, this.expressions, parsedTimes, ValidModes.none).sort();
         }
         if (parsedTimesIndexes.length == 0) {
            return parsedTimes;
         }
