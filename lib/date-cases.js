 ...
 */
function splitContext(expressions) {
...
const numberAndWordParseCases = [
    new ParseCase(numberAndWordPrevalence, function findSeconds(parsedTimes, prevalence) {
        ...
        for (const match of matches) {
            ...
            if (num <= max || max == 0) {
                parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.seconds, num, this.expressions[match.index + 1].value, indexes, prevalence));
            }
        }
        return parsedTimes;
    }),
    new ParseCase(numberAndWordPrevalence, function findMinutes(parsedTimes, prevalence) {
        ...
        for (const match of matches) {
            ...
            let max = this.expressions[match.index + 1].maximum
            if (num <= max || max == 0) {
                parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.minutes, num, this.expressions[match.index + 1].value, indexes, prevalence));
            }
        }
        return parsedTimes;
    }),
    new ParseCase(numberAndWordPrevalence, function findHours(parsedTimes, prevalence) {
        ...
        for (const match of matches) {
            ...
            let max = this.expressions[match.index + 1].maximum
            if (num <= max || max == 0) {
                parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.hours, num, this.expressions[match.index + 1].value, indexes, prevalence));
            }
        }
        return parsedTimes;
    }),
    new ParseCase(numberAndWordPrevalence, function findDates(parsedTimes, prevalence) {
        ...
        for (const match of matches) {
            ...
            let max = this.expressions[match.index + 1].maximum
            if (num <= max || max == 0) {
                parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.dates, num, this.expressions[match.index + 1].value, indexes, prevalence));
            }
        }
        return parsedTimes;
    }),
    new ParseCase(numberAndWordPrevalence, function findMonths(parsedTimes, prevalence) {
        ...
        for (const match of matches) {
            ...
            let max = this.expressions[match.index + 1].maximum
            if (num <= max || max == 0) {
                parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.months, num, this.expressions[match.index + 1].value, indexes, prevalence));
            }
        }
        return parsedTimes;
    }),
    new ParseCase(numberAndWordPrevalence, function findYears(parsedTimes, prevalence) {
        ...
        for (const match of matches) {
            ...
            let max = this.expressions[match.index + 1].maximum
            if (num <= max || max == 0) {
                parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.years, num, this.expressions[match.index + 1].value, indexes, prevalence));
            }
        }
        return parsedTimes;
    }),
    ...
    new ParseCase(100, function (parsedTimes, prevalence) {
        ...
        for (const match of matches) {
            ...
            if (checkHours(hours) && checkMinutes(minutes)) {
                ...
                parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.hours, hours, 0, indexes, prevalence));
                parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.minutes, minutes, 0, indexes, prevalence));
            }
        }
        return parsedTimes;
    })
...
function extractTime(expressions, minimumPrevalence) {
    ...
    let contexts = splitContext(expressions);
