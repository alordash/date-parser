 ...
 * @returns {Number} 
 */
function findCorrespondingContext(indexes) {
    ...
    while (i--) {
        ...
        while (j--) {
            ...
            if (start <= index && index <= end) {
                if (!this.usedContexts.includes(j)) {
                    this.usedContexts.push(j);
                }
                return j;
            }
        }
    ...
    new ParseCase(numberAndWordPrevalence, function findSeconds(parsedTimes, contextsData, prevalence) {
        ...
        for (const match of matches) {
            ...
            if (num <= max || max == 0) {
                ...
                let context = findCorrespondingContext.call(contextsData, indexes);
...
const parseCases = [
    ...
    new ParseCase(100, function (parsedTimes, contextsData, prevalence) {
 ...
 * @returns {{parsedTimes: Array.<ParsedTime>, contextsData: ContextsData}} object containing all found times 
 */
function extractTime(expressions, minimumPrevalence) {
    ...
    contextsData.contexts = splitContext.call(expressions);
    ...
    return { parsedTimes, contextsData };
}
