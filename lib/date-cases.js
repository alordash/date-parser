...
const parseCases = [
   ...
   new ParseCase(90, function (parsedTimes, contextsData, prevalence) {
      ...
      for (const match of matches) {
         ...
         let context = processContexts(contextsData, indexes);
         const now = new Date();
         parsedTimes.push(new ParsedTime(DateTypes.target, TimeTypes.dates, now.getDate() + value, indexes, context, prevalence, { isOffset: true, validMode: ValidModes.certified }));
      }
      return parsedTimes;
   }),
   //Searchs for "after 'X'"
